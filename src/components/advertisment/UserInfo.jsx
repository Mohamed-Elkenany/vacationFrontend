  ...
  return (
   <div className="w-full h-full bg-white dark:bg-gray-800 rounded-md shadow-md flex-1 flex flex-col justify-between">
      ...
      {isSuccess && (
        <Link
          to={`/profile/${userId}`}
          className="relative w-full flex-1 max-h-full rounded-md  flex flex-col justify-between"
        >
          <div
            className={`${
              banner?.url ? "px-0" : "p-1 rounded-lg"
            } relative rounded-t-md w-full h-[150px]`}
          >
            {banner?.url ? (
              <img
                src={banner.url}
                alt="user_banner"
                className="rounded-t-md w-full h-full object-cover object-top"
              />
            ) : (
              <div className="w-full h-full bg-slate-100 dark:bg-slate-900">
                <h1 className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-xl font-lobster tracking-widest text-gray-700 dark:text-gray-400">
                  Vacation
                </h1>
              </div>
            )}
          ...
          <div className="my-4 py-2">
            <div>
              <h1 className="text-center font-lobster tracking-widest text-purple-700 dark:text-slate-300 max-xl:text-sm font-semibold">{`${user?.userName[0].toUpperCase()}${user?.userName
                .split(" ")[0]
                .slice(1)} ${
                ...
                user?.userName.split(" ")[1] !== undefined
                  ? user?.userName.split(" ")[1].slice(1)
                  : ""
              }`}</h1>
            </div>
            {user?.bio && (
              <div className="flex items-start gap-1 text-purple-700 dark:text-slate-400 text-xs font-lobster tracking-wider pl-2 pr-1 max-xl:py-1 mt-2">
            ...
            <div className="border-t dark:border-slate-600 flex items-center justify-around py-1">
              <div className="font-lobster tracking-wider text-purple-900 dark:text-slate-200 flex flex-col items-center justify-center border-r dark:border-slate-600 w-full h-full">
                <span className="text-sm font-semibold">
                  {toggleFollow?.user?.followers?.length > 0
                    ? toggleFollow?.user?.followers?.length
                    : "0"}
                </span>
                <span className="text-sm max-xl:text-xs font-semibold">
                  Followers
                </span>
              </div>
              <div className="font-lobster tracking-wider text-purple-900 dark:text-slate-200 flex flex-col items-center justify-center border-r dark:border-slate-600 w-full h-full">
                <span className="text-sm font-semibold">
                  {toggleFollow?.user?.following?.length > 0
                    ? toggleFollow?.user?.following?.length
                    : "0"}
                </span>
                <span className="text-sm max-xl:text-xs font-semibold">
                  Following
              ...
              <div className="font-lobster tracking-wider text-purple-900 dark:text-slate-200 flex flex-col items-center justify-center  w-full h-full">
                <span className="text-sm font-semibold">
                  {countOfPost?.length > 0 ? countOfPost.length : "0"}
                </span>
                <span className="text-sm max-xl:text-xs font-semibold">
                  Posts
                </span>
