...
const HomePage = ({ socket }) => {
  ...
  const handleEditBio = async (e) => {
    e.preventDefault();
    await UpdateBIO({ userId: USER._id, userName: USER.userName, bio: mainBIO })
      .then(res => res.data)
    ...
    <div className='homePage max-md:px-0 w-full min-h-screen dark:bg-transparent px-2 flex'>
      {
        ...
        <div className='absolute z-[1000] left-0 top-0 min-h-full w-full bg-black bg-opacity-70 '>
          <div className=' max-md:w-11/12 md:w-1/3 max-h-[500px] rounded-sm bg-white dark:bg-gray-700 fixed left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 py-1 px-2'>
            <button disabled={isLoading} className='text-white absolute -right-4 -top-4 bg-black w-7 h-7 rounded-sm flex items-center justify-center' onClick={() => setUpdateBIO(false)}>X</button>
            <div className=' bg-transparent w-full max-h-[490px] overflow-y-scroll scrollbar-none'>
              <h1 className="text-center underline text-gray-900 dark:text-slate-300 font-lobster tracking-wider">Edit BIO</h1>
              <form onSubmit={handleEditBio}>
                <div className='flex flex-col items-center my-2 py-1 dark:text-slate-300'>
                  <textarea rows="5" defaultValue={mainBIO} placeholder="BIO..." className='w-full bg-transparent pl-2 outline-none border dark:border-gray-600 resize-none max-h-[60px] mb-1 scrollbar-none' onChange={(e) => setMainBIO(e.target.value)}></textarea>
                  <button disabled={isLoading} className='bg-purple-700 px-2 font-lobster tracking-wider rounded-sm'>Edit</button>
                </div>
              </form>
            </div>
          </div>
